...
m2 <- dat %>% meta() %>% dplyr::select(donor_group, Paired, id)
...
process_meta(dat)
mag3 <- mag2 %>% dplyr::select(c(id, value)) %>% 
...
v1 <- ggplot(data=mag2, aes(x=description, y=value)) +
  ...
  geom_point(aes(fill = description), position = position_jitterdodge(jitter.width = 1), 
  ...
  geom_signif(comparisons = list(c("PD Patient", "Household Control")),
              annotations = sig_mapper(observed.PdHC.pval)) +
  geom_signif(comparisons = list(c("Population Control", "PD Patient")),
...
ggsave(v1, filename = "figures/Figure_4/Total_Virulence_Factors_VFDB.svg",
       height = 5,
       width = 4)
...
title =  " ";
  ...
  mag4pcoa <- mag2 %>% dplyr::select(c(id, value)) %>% 
