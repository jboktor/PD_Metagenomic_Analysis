...
g1 <- df.plot.all %>%
  gather(
    key = type, value = meta, -species, -disease,
...
g2 <- df.plot.study %>%
  ...
  ylab(paste0(
    "Variance explained by Household Effect \n average: ",
    formatC(mean(df.plot.study$meta) * 100, digits = 2), "%"
  )) +
  theme_bw() +
  theme(panel.grid.minor = element_blank()) +
  scale_fill_manual(
    values = c("grey", "#CC071E"),
    name = paste0("Significance\n(", alpha.meta, " FDR)")
  ) +
  ...
  geom_text_repel(
    data = subset(df.plot.study, meta.significance),
