...
library(ggbeeswarm);library(FSA);library(ggpubr);library(ggsci);library(microbiome);library(ggridges);library(future);library(cowplot); 
library(scales)
...
ggsave(merge1, filename = "MaaslinQC_pqvalue_Plots.png",
       width = 14, height = 12)

merge3 <- plot_grid(p3, p4, align = c("h"), nrow = 1)
ggsave(merge3, filename = "MaaslinQC_prevalence-pvalue_Plots.png",
       width = 14, height = 12)
...
ggsave(merge2, filename = "MaaslinQC_Coef-pvalue_Plots.png",
       width = 12, height = 12)
