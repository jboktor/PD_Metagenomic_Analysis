...
Maas.pd.pc <- read_tsv(
  paste0("data/MaAsLin2_Analysis/Merged/", datName, "_PDvPC_maaslin2_output/all_results.tsv"),
  col_names = T
) %>%
  filter(value == "Population Control")
Maas.pd.hc <- read_tsv(
  paste0("data/MaAsLin2_Analysis/Merged/", datName, "_PDvHC_maaslin2_output/all_results.tsv"),
  col_names = T
) %>%
