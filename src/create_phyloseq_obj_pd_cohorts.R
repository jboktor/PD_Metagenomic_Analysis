...
KOs.abund <- dat.KOs.slim %>% abundances() %>% as.data.frame() %>% 
  mutate_all(~ tss(.)) %>% 
  rownames_to_column() %>% 
...
# Re group and re-normalize 
GOs.abund <- dat.GOs.slim %>% abundances() %>% as.data.frame() %>% 
  mutate_all(~ tss(.)) %>% 
  rownames_to_column() %>% 
  filter(!grepl("UNMAPPED", rowname)) %>%
  filter(!grepl("UNGROUPED", rowname)) %>% 
...
# Re group and re-normalize 
PFAMs.abund <- dat.PFAMs.slim %>% abundances() %>% as.data.frame() %>% 
  mutate_all(~ tss(.)) %>% 
  rownames_to_column() %>% 
  filter(!grepl("UNMAPPED", rowname)) %>%
  filter(!grepl("UNGROUPED", rowname)) %>% 
...
# Re group and re-normalize 
EGGNOGs.abund <- dat.EGGNOGs.slim %>% abundances() %>% as.data.frame() %>% 
  mutate_all(~ tss(.)) %>% 
  rownames_to_column() %>% 
  filter(!grepl("UNMAPPED", rowname)) %>%
  filter(!grepl("UNGROUPED", rowname)) %>% 
