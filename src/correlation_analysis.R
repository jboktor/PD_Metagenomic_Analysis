...
se.mb.amgut <- spiec.easi(amgut1.filt,
  method = "mb", lambda.min.ratio = 1e-2,
  nlambda = 20, pulsar.params = list(rep.num = 50)
)
se.gl.amgut <- spiec.easi(amgut1.filt,
  method = "glasso", lambda.min.ratio = 1e-2,
  nlambda = 20, pulsar.params = list(rep.num = 50)
)
...
plot(0:(length(dd.sparcc) - 1), dd.sparcc,
  ylim = c(0, .35), type = "b",
...
data(hmp2)
se.hmp2 <- spiec.easi(list(hmp216S, hmp2prot),
...
se.mb.amgut2 <- spiec.easi(dat.plot,
  method = "mb", lambda.min.ratio = 1e-2,
  nlambda = 20, pulsar.params = list(rep.num = 50)
)
