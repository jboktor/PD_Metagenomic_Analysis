...
ml_s2s <- function(Shanghai,
  ...
  if (!is.null(featSelection)) {
    ...
    featSelect_Shanghai <- feature_selection_auroc(
      df = aucs, n_features = nfeats,
      subgroup = "Shanghai", data_type = data_type
    )[1:nfeats]
    featSelect_TBC <- feature_selection_auroc(
      df = aucs, n_features = nfeats,
      subgroup = "TBC", data_type = data_type
    )[1:nfeats]
    featSelect_Rush <- feature_selection_auroc(
      df = aucs, n_features = nfeats,
      subgroup = "Rush", data_type = data_type
    )[1:nfeats]
    featSelect_Bonn <- feature_selection_auroc(
      df = aucs, n_features = nfeats,
      subgroup = "Bonn", data_type = data_type
    )[1:nfeats]
    # MRMR methods
...
ml_loso <- function(Shanghai,
  ...
  if (!is.null(featSelection)) {
    featselectStart <- Sys.time()
    featSelect_noShanghai <- feature_selection_auroc(
      df = aucs, n_features = nfeats,
      subgroup = "no_Shanghai", data_type = data_type
    )[1:nfeats]
    featSelect_noTBC <- feature_selection_auroc(
      df = aucs, n_features = nfeats,
      subgroup = "no_TBC", data_type = data_type
    )[1:nfeats]
    featSelect_noRush <- feature_selection_auroc(
      df = aucs, n_features = nfeats,
      subgroup = "no_Rush", data_type = data_type
    )[1:nfeats]
    featSelect_noBonn <- feature_selection_auroc(
      df = aucs, n_features = nfeats,
      subgroup = "no_Bonn", data_type = data_type
    )[1:nfeats]
    # mRMR_noShanghai <-
    ...
    names(feats) <- c(
      "featSelect_noShanghai", "featSelect_noTBC",
...
lasso_cohort_x_group_summary <- function(obj_tbc_all, obj_rush_all, featSelection = NULL) {
  ...
  ml_input_tbc_house <- obj_tbc_all %>%
    subset_samples(paired != "No") %>%
    prep_ml_input()
  ml_input_tbc_healthy <- obj_tbc_all %>%
    subset_samples(donor_group != "HC") %>%
    prep_ml_input()
  ...
  rush_PC_vs_rush_HC <- lasso_model(
    train = ml_input_rush_healthy,
    test = ml_input_rush_house
  )
  rush_PC_vs_tbc_PC <- lasso_model(
    train = ml_input_rush_healthy,
    test = ml_input_tbc_healthy
  )
  rush_PC_vs_tbc_HC <- lasso_model(
    train = ml_input_rush_healthy,
    test = ml_input_tbc_house
  )

  rush_HC_vs_rush_PC <- lasso_model(
    train = ml_input_rush_house,
    test = ml_input_rush_healthy
  )
  rush_HC_vs_tbc_PC <- lasso_model(
    train = ml_input_rush_house,
    test = ml_input_tbc_healthy
  )
  rush_HC_vs_tbc_HC <- lasso_model(
    train = ml_input_rush_house,
    test = ml_input_tbc_house
  )

  tbc_PC_vs_tbc_HC <- lasso_model(
    train = ml_input_tbc_healthy,
    test = ml_input_tbc_house
  )
  tbc_PC_vs_rush_PC <- lasso_model(
    train = ml_input_tbc_healthy,
    test = ml_input_rush_healthy
  )
  tbc_PC_vs_rush_HC <- lasso_model(
    train = ml_input_tbc_healthy,
    test = ml_input_rush_house
  )

  tbc_HC_vs_tbc_PC <- lasso_model(
    train = ml_input_tbc_house,
    test = ml_input_tbc_healthy
  )
  tbc_HC_vs_rush_PC <- lasso_model(
    train = ml_input_tbc_house,
    test = ml_input_rush_healthy
  )
  tbc_HC_vs_rush_HC <- lasso_model(
    train = ml_input_tbc_house,
    test = ml_input_rush_house
  )
