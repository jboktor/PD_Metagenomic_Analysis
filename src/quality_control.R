...
c1
ggsave(c1,
  filename = "data/Community_Composition/Quality_Control/Sequencing_Depth_Density_&_ECDF_Merged_Facet.png",
...
ggsave(p2,
  filename = "data/Community_Composition/Quality_Control/Sequencing_Depth_Boxplt.svg",
  width = 3, height = 6
)
...
env <- mutate(env,
  clean_total_reads_factor_tight =
...
# z1
ggsave(z1,
  filename = "data/Community_Composition/Quality_Control/SeqDepth_&_Abundance_Quantile_Histogram_Matrix.png",
...
# z2
ggsave(z2,
  filename = "data/Community_Composition/Quality_Control/SeqDepth_&_Abundance_Quantile_ECDF_matrix.png",
  width = 20, height = 8, dpi = 1200
)
...
z1b <- abm %>%
  filter(abundance_factor == 2 & clean_total_reads_factor_tight == 1) %>%
  ggplot(aes(x = value, color = donor_group, fill = donor_group), alpha = 0.4) +
  geom_histogram(position = "dodge", boundary = 0) +
  facet_grid(
    cols = vars(clean_total_reads_factor_tight),
...
ggsave(QC_speciesAbundanceVsreads,
  filename = "data/Community_Composition/Quality_Control/SeqDepth_Quantile_Distribution.png",
...
for (i in obj) {
  ...
  observed.plot <- AlphaLinearRegressionQuantilePlot(
    df = env, x = env$clean_total_reads, x2 = env$clean_total_reads_factor, y = env$observed,
  ...
  shannon.plot <- AlphaLinearRegressionQuantilePlot(
    df = env, x = env$clean_total_reads, x2 = env$clean_total_reads_factor, y = env$shannon,
    color = env$donor_group, fill = env$donor_group,
    ylabel = paste("Shannon Diversity:", obj.label[cnt]), title = "Shannon Index by Read Depth"
  )

  evenness.plot <- AlphaLinearRegressionQuantilePlot(
    df = env, x = env$clean_total_reads, x2 = env$clean_total_reads_factor, y = env$evenness,
    color = env$donor_group, fill = env$donor_group,
  ...
  ggsave(QC_alphaVsreads,
    filename = paste0("data/Community_Composition/Quality_Control/Read_Depth_vs_Alpha_Diversity/Read_Depth_vs_", obj.label[cnt], "_AlphaDiversity.png"),
    width = 16, height = 8
  )
...
for (i in obj) {
  ...
  QC_betaVsreads <- BetaLinearRegressionPlot(
    df = df.pcoa, x = df.pcoa$clean_total_reads, y = df.pcoa$Axis.1, y2 = df.pcoa$Axis.2,
  ...
  ggsave(QC_betaVsreads,
    filename = paste0("data/Community_Composition/Quality_Control/Read_Depth_vs_Beta_Diversity/Read_Depth_vs_", obj.label[cnt], "_BetaDiversity.png"),
    width = 8, height = 8
  )
