...
metabolism_overview <- 
  ...
  geom_point(aes(fill = group, color = group), position = position_jitterdodge(jitter.width = 0.2), 
             shape=21, size=1.5, alpha = 0.6) +
  theme_bw() +
  geom_text(data=filter(metabolism_df3, group=="PC"), 
            aes(x=feature, y= (Max.values + Max.values*0.15), 
                label = qval.print), size = 2.9, check_overlap = TRUE, nudge_x = -0.25) +
  geom_text(data=filter(metabolism_df3, group=="HC"), 
            aes(x=feature, y= (Max.values + Max.values*0.15), 
                label = qval.print), size = 2.9, check_overlap = TRUE, nudge_x = 0.25) +
