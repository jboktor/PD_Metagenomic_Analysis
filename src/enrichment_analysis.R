...
KO_BRITE <-  fromJSON("files/enrichment_analysis/ko00001_BRITE.json") %>%
  ...
  unnest(children, names_repair = "unique") %>%
  dplyr::rename(hierarchy = children.name,
                level_1 = name...3,
                level_2 = name...4,
                level_3 = name...5) %>%
...
KO_BRITE_keys <- KO_BRITE %>% 
  select(hierarchy, level_1, level_2) %>% distinct() %>% 
...
for (path in unique(annot_KOs_pdpc$level_2)) {
  ...
  total_sig_lib <- annot_KOs_pdpc %>% filter(qval <= 0.25, coef < 0) %>%  pull(KOs) %>% unique() %>% length()
  path_lib <- annot_KOs_pdpc %>% filter(level_2 ==  path) %>%  pull(KOs) %>% unique() %>% length()
  path_sig_lib <- annot_KOs_pdpc %>% filter(level_2 ==  path, qval <= 0.25, coef < 0) %>%  pull(KOs) %>% unique() %>% length()
...
for (path in unique(annot_KOs_pdpc$level_2)) {
  ...
  total_lib <- length(unique(annot_KOs_pdpc$KOs))
  total_sig_lib <-
    annot_KOs_pdpc %>% filter(qval <= 0.25, coef > 0) %>%  pull(KOs) %>% unique() %>% length()
  path_lib <-
    annot_KOs_pdpc %>% filter(level_2 ==  path) %>%  pull(KOs) %>% unique() %>% length()
  path_sig_lib <-
    annot_KOs_pdpc %>% filter(level_2 ==  path, qval <= 0.25, coef > 0) %>%  pull(KOs) %>% unique() %>% length()
...
for (path in unique(annot_KOs_pdhc$level_2)) {
  ...
  total_sig_lib <- annot_KOs_pdhc %>% filter(qval <= 0.25, coef < 0) %>%  pull(KOs) %>% unique() %>% length()
  path_lib <- annot_KOs_pdhc %>% filter(level_2 ==  path) %>%  pull(KOs) %>% unique() %>% length()
  path_sig_lib <- annot_KOs_pdhc %>% filter(level_2 ==  path, qval <= 0.25, coef < 0) %>%  pull(KOs) %>% unique() %>% length()
...
metabolism_fig_plot <- 
  ...
  geom_point(data = metabolism_fig.s, 
             aes(y = enrichment_pdhc, x = enrichment_pdpc, size = km, 
                 fill = level_1), 
  ...
  geom_text_repel(data = metabolism_fig.lab,
  ...
  labs(x="Enrichment Scores (PD / PC)", y ="Enrichment Scores (PD / HC)",
...
ggsave(metabolism_fig_plot, filename = "data/Enrichment_Analysis/Kegg_Metabolism_bubbleplot_new2.svg",
       width = 9, height = 6)
...
kegg_fig_plot <- 
  ...
  geom_point(aes(x = enrichment_pdhc, y = enrichment_pdpc, size = km), 
             fill = "#525252", alpha = 0.2) +
  geom_point(data = kegg_fig.s, 
             aes(x = enrichment_pdhc, y = enrichment_pdpc, size = km, 
                 fill = level_1), 
             alpha = 0.9, shape=21, stroke = 0.25) +
  ...
  geom_text_repel(data = kegg_fig.lab,
                  ...
                  size = 2.75) +
  facet_wrap(~direction, scales = "free", ncol = 1) +
  labs(y="Enrichment Scores (PD / PC)", x ="Enrichment Scores (PD / HC)",
       fill = "KEGG Hierarchy", size = "Number of KOs with \nq<0.25 in group") +
  theme_bw() +
  guides(fill = guide_legend(override.aes = list(size=3))) +
  theme(panel.grid = element_blank())
...
save_me_cleanly(ggobj = kegg_fig_plot,
