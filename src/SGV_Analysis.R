...
r1 <- 
  ...
  ggplot(aes(x= reorder(sv, PD_deletion_ratio),
             y= log2(PD_deletion_ratio), 
             color=p.value)) +
  labs(x="Ranked Structural Variants", 
...
v1 <- 
  pdhc.dsgv.stats.plot %>% 
  ggplot(aes(x= log2(PD_deletion_ratio),
             y= -log10(p.value), 
             fill=p.value)) +
  labs(x=expression(paste(log[2], "[ PD/HC detection-ratio ]")),
       y=expression(paste(-log[10], "[ p-value ]")),
       fill = "p-value") +
  scale_fill_viridis_c(option="magma", direction = -1,
                       na.value = "#000000",
                       limits=c(0, 0.25)) +  geom_point(shape=21) +
  geom_text_repel(data = pdhc.dsgv.stats.plot %>%
                    filter(lab == "yes"),
                  aes(x= log2(PD_deletion_ratio),
...
v2 <- 
  ...
  ggplot(aes(x= log2(PD_deletion_ratio),
             y= -log10(p.value), 
             fill=p.value)) +
  labs(x=expression(paste(log[2], "[ PD/PC detection-ratio ]")),
       y=expression(paste(- log[10], "[ p-value ]")),
       fill = "p-value") +
  scale_fill_viridis_c(option="magma", direction = -1,
                        na.value = "#000000",
                        limits=c(0, 0.25)) +
  geom_point(shape=21) +
  geom_text_repel(data = pdpc.dsgv.stats.plot %>% 
                    filter(lab == "yes"),
                  aes(x= log2(PD_deletion_ratio),
                      y= -log10(p.value), label=sv), 
...
c1 <- cowplot::plot_grid(r1, v1, align = "hv", axis = "lrtb", 
                         ncol = 1, rel_heights = c(1, 2.25))
c2 <- cowplot::plot_grid(r2, v2, align = "hv", axis = "lrtb", 
                         ncol = 1, rel_heights = c(1, 2.25))
c3 <- cowplot::plot_grid(c1, c2, nrow = 1, align = "hv")


ggsave(c3, filename = "data/SGVFinderViz/deletion_sgvs.png",
