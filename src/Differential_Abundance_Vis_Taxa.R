...
g0 <- ggplot() +
  geom_col(data=PDovrPC.BP, aes(x=gFC, y= feature,  fill = direction), 
...
g3 <- ggplot() +
  geom_col(data=dat_pdpc.PREV, aes(x=value*100, y= feature, fill = variable), 
...
h0 <- ggplot() +
  geom_col(data=PDovrHC.BP, aes(x=gFC, y= feature,  fill = direction), 
           position = "dodge", width = 0.6, alpha = 0.5) +
  theme_minimal() +
  labs(x="Generalized Fold Change") +
  # ggtitle("Fold Change") +
...
DAF_part1 <- cowplot::plot_grid(g1a, h1a, nrow = 2, align = "hv", 
...
ggsave(DAF_final, filename = paste0("data/DAF_Analysis/DAF_", lev, "_MaaslinSig.png"),
       width = 20, height = 10)
