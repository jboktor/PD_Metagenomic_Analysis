...
feature_accumulation_plot <- function(dat, featuretype = "Species", reads, cohort) {
  ...
  # Filter
  psudocnts.HC <- psudocnts %>%
    dplyr::select(contains("HC")) %>%
    t() %>%
    as.data.frame()
  psudocnts.PC <- psudocnts %>%
    dplyr::select(contains("PC")) %>%
    t() %>%
    as.data.frame()
  psudocnts.PD <- psudocnts %>%
    dplyr::select(!contains(c("HC", "PC"))) %>%
    t() %>%
    as.data.frame()
  ...
  ggsave(p1,
    filename =
      paste0(
        "data/Community_Composition/Rarefaction_Plots/Rarefaction_Plot_", cohort, "_",
...
AlphaLinearRegressionQuantilePlot <- function(df, x, x2, y, color, fill, ylabel, title) {
  ...
  p1 <- ggplot(df, aes(x = x, y = y, color = color, fill = fill)) +
    ...
    stat_cor(aes(
      ...
      color = color
    ), label.x = 15e6, label.y.npc = 0.25, hjust = 0) +
    ggtitle(title) +
...
BetaLinearRegressionPlot <- function(df, x, y, y2, color, fill, feature, title) {
  ...
  p1 <- ggplot(df, aes(x = x, y = y, color = color, fill = fill)) +
    ...
    stat_cor(aes(
      label = paste(..rr.label.., ..p.label.., sep = "~`,`~"),
      color = color
    ), label.x = 1.75e7, label.y.npc = 0.25, hjust = 0) +
    ggtitle(title) +
    scale_fill_manual(values = c("HC" = HC.col, "PD" = PD.col, "PC" = PC.col)) +
    scale_color_manual(values = c("HC" = HC.col, "PD" = PD.col, "PC" = PC.col), guide = FALSE) +
    theme(
...
DMM_fit <- function(datObj, nmax) {
  ...
  dmm.fit.plot <-
    ...
    scale_color_d3() +
    labs(
      x = "Number of Dirichlet Components", y = "Model Fit",
      color = "Model Metrics"
    ) +
    theme_classic() +
