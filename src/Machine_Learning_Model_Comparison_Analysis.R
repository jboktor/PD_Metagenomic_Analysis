...
for (i in x) {
  ...
  rocplot <- ggplot() +
    ...
    geom_roc(
      data = ridge.PDHC$optimal.df, aes(m = PD, d = factor(obs, levels = c("PD", "HC"))),
      n.cuts = 0, color = cols.ml[1], linealpha = 0.8
    ) +
    geom_roc(
      data = ridge.PDPC$optimal.df, aes(m = PD, d = factor(obs, levels = c("PD", "PC"))),
      n.cuts = 0, color = cols.ml[2], linealpha = 0.8, labels = TRUE
    ) +
    geom_roc(
      data = lasso.PDHC$optimal.df, aes(m = PD, d = factor(obs, levels = c("PD", "HC"))),
      n.cuts = 0, color = cols.ml[3], linealpha = 0.8
    ) +
    geom_roc(
      data = lasso.PDPC$optimal.df, aes(m = PD, d = factor(obs, levels = c("PD", "PC"))),
      n.cuts = 0, color = cols.ml[4], linealpha = 0.8
    ) +
    geom_roc(
      data = enet.PDHC$optimal.df, aes(m = PD, d = factor(obs, levels = c("PD", "HC"))),
      n.cuts = 0, color = cols.ml[5], linealpha = 0.8
    ) +
    geom_roc(
      data = enet.PDPC$optimal.df, aes(m = PD, d = factor(obs, levels = c("PD", "PC"))),
      n.cuts = 0, color = cols.ml[6], linealpha = 0.8
    ) +
    geom_roc(
      data = rf.PDHC$optimal.df, aes(m = PD, d = factor(obs, levels = c("PD", "HC"))),
      n.cuts = 0, color = cols.ml[7], linealpha = 0.8
    ) +
    geom_roc(
      data = rf.PDPC$optimal.df, aes(m = PD, d = factor(obs, levels = c("PD", "PC"))),
      n.cuts = 0, color = cols.ml[8], linealpha = 0.8
    ) +
    geom_roc(
      data = xgboost.PDHC$optimal.df, aes(m = PD, d = factor(obs, levels = c("PD", "HC"))),
      n.cuts = 0, color = cols.ml[9], linealpha = 0.8
    ) +
    geom_roc(
      data = xgboost.PDPC$optimal.df, aes(m = PD, d = factor(obs, levels = c("PD", "PC"))),
  ...
  ml.models <- list(
    ridge.PDHC, ridge.PDPC, lasso.PDHC, lasso.PDPC, enet.PDHC, enet.PDPC,
  ...
  cols.ml.labels <- c(
    "ridge.PDHC" = "Ridge HC", "ridge.PDPC" = "Ridge PC",
  ...
  ggsave(finalplot,
    filename = paste0("data/Machine_Learning_Analysis/Model_Comparisons_", z[cnt], ".svg"),
    width = 11.5, height = 5
  )
