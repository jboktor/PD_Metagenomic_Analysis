...
gmm <- rpm(input_kos,
  minimum.coverage = 0.3, annotation = 1,
...
df.gbm <- data.frame(
  row.names = translated_annotations_gbm,
...
df.gmm <- data.frame(
  row.names = translated_annotations_gmm,
...
plot_daf_summary(
  obj.name = "GBMs", obj = dat.GBMs, cohort = "Merged",
  repelyup = 0.002, repelydn = 0.002
)

plot_dafs(obj.name = "GMMs", obj = dat.GMMs, cohort = "Merged")
plot_daf_summary(
  obj.name = "GMMs", obj = dat.GMMs, cohort = "Merged",
