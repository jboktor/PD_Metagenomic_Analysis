...
mdsupdrs_III <-
  c(
    "mds_updrs_3_arising_from_chair",
...
clinical_variables <-
  c(
    "family_history_pd_degree_relative",
    "olfactory_diagnosis",
    "smell_score_upsit",
...
process_meta <- function(dat, cohort, verbose = F) {
  if (cohort == "TBC") {
    ...
    env <-
      env %>%
      mutate(
        across(all_of(diet), as.character),
        across(all_of(diet), as.numeric)
      ) %>%
      mutate(
        across(all_of(mdsupdrs_I.II), as.character),
    ...
    env <-
      ...
      mutate(
        across(all_of(motor_severity_scores_summary), as.character),
        across(all_of(motor_severity_scores_summary), as.numeric)
      ) %>%
      mutate(
        across(all_of(clinical_variables), as.character),
...
process_meta_study_design_plot <- function(dat) {
  ...
  diet <- c(
    "breakfast_frequency", "meat_consumption_frequency", "fish_consumption_frequency",
