...
corrs.clinical %>% 
  ...
  scale_color_viridis_c(option = "magma", begin = 0, end = 1, direction = -1,
  na.value = "transparent") +
...
fillpal <- c('GOs'= '#a6cee3','KOs'='#1f78b4','PFAMs'='#b2df8a',
...
corrs.sig.barplot <- 
  ...
  labs(y = NULL, x = expression(paste("log"[ 10]*" (FDR < 0.2)")),
       fill = "Feature level") +
  scale_fill_manual(values = fillpal) +
  facet_grid(row = vars(category), scales = "free_y", space = "free_y",
...
ggsave(corrs.sig.barplot, filename = "data/Correlations/Summary.svg", 
       width = 6.5, height = 5)
