...
daf_boxplot_sigvalues <- function(sigplot.df, abund.input) {
  ...
  sigplot.df.QVAL <- sigplot.df.QVAL %>%
    dplyr::select(feature, sig.labels) %>%
...
daf_boxplots <- function(df, fill_cols, rim_cols, alfa = 0.5, obj.name) {
  ...
  p <- ggplot(data = df, aes(x = value, y = Var2)) +
    ...
    geom_text(aes(
      x = max(value) + max(value) * 0.15,
      y = Var2, label = sig.labels
...
prevalence_barplot <- function(df, manual_colors, alfa = 0.5) {
  p <- ggplot(data = df, aes(x = value * 100, y = feature, fill = variable, color = variable)) +
    geom_bar(
      position = position_dodge(), alpha = alfa, width = 0.8,
      stat = "identity"
    ) +
    theme_minimal() +
...
variance_plot <- function(dat) {
  ...
  filter.val <- apply(int.mat, 1, function(x) {
    diff(quantile(as.numeric(x), c(0.1, 0.9),
  ...
  p <- ggplot(var.df, aes(x = reorder(features, -filter.val), y = log10(abs(filter.val) + 1e-10))) +
    ...
    geom_vline(xintercept = c(
      rk[rk == round(rws * .9)], rk[rk == round(rws * .8)], rk[rk == round(rws * .7)],
      rk[rk == round(rws * .6)], rk[rk == round(rws * .5)], rk[rk == round(rws * .4)],
      rk[rk == round(rws * .3)], rk[rk == round(rws * .2)]
...
plot_dafs <- function(obj.name, obj, cohort = "TBC", tag = "") {
  ...
  if (!is.null(v$venn_depleted)) {
    pdf(
      file = paste0("data/DAF_Analysis/", cohort, "/DAF_", obj.name, "_VennDiagram_PD_depleted.pdf"),
      width = 7,
      height = 5,
      pointsize = 12
    )
  ...
  if (!is.null(v$venn_enriched)) {
    pdf(
      file = paste0("data/DAF_Analysis/", cohort, "/DAF_", obj.name, "_VennDiagram_PD_enriched.pdf"),
      width = 7,
      height = 5,
      pointsize = 12
    )
  ...
  gfc_data <- generalized_fold_change(
    pd_abundance = dat_pdhc.PDabun,
...
plot_daf_summary <- function(obj.name, obj, cohort = "TBC", tag = "",
  ...
  ggsave(gFC_diff_plot,
    filename = paste0("data/DAF_Analysis/", cohort, "/DAF_summary_", obj.name, tag, ".svg"),
    width = 6.5, height = 5
  )
}
