...
alpha_diversity_summary <- function(cohort) {
  ...
  for (i in x) {
    ...
    p1 <- alpha_div_boxplots(
      df = env, x = env$donor_group, y = env$Observed,
    ...
    p2 <- alpha_div_boxplots(
      df = env, x = env$donor_group, y = env$Shannon,
    ...
    p3 <- alpha_div_boxplots(
      df = env, x = env$donor_group, y = env$Evenness,
    ...
    ggsave(alpha_cow,
      filename =
        paste0(
          "data/Community_Composition/Alpha_Diversity_Analysis/AlphaDiversity_BoxPlot_",
