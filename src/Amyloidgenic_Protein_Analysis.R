...
phyloseq_objs <- readRDS("files/Phyloseq_Merged/PhyloseqObj_clean.rds") #phyloseq_objs
...
env <- phyloseq_objs[["Species"]] %>% 
  meta() %>% 
  select(all_of(c(Anthro, environmental, grouping)), donor_id, donor_group) %>% 
  mutate(description = factor(description, levels = names(plot.cols)))
env %>% glimpse

KEGG_curli_data <- 
  phyloseq_objs[["KOs.slim"]] %>% 
  abundances() %>% 
  microbiome::transform("compositional") %>%
  as.data.frame() %>% 
  rownames_to_column() %>% 
  decode_rfriendly_rows("rowname") %>% 
  filter(grepl("curli", fullnames, ignore.case = T) | grepl("csg", fullnames, ignore.case = T)) %>% 
  select(-rowname) %>% 
  pivot_longer(!fullnames, names_to = "donor_id", values_to = "relative_abundance") %>% 
  mutate(arcsin_sqrt_relative_abundance = asin(sqrt(relative_abundance))) %>% 
  left_join(env) %>% 
  dplyr::rename(feature = fullnames)
KEGG_curli_data %>% glimpse


Pfam_curli_data <- 
  phyloseq_objs[["Pfams.slim"]] %>% 
  abundances() %>% 
  microbiome::transform("compositional") %>%
  as.data.frame() %>% 
  rownames_to_column() %>% 
  ...
  left_join(env) %>% 
  dplyr::rename(feature = fullnames)
Pfam_curli_data %>% glimpse


curli_features <- bind_rows(KEGG_curli_data, Pfam_curli_data)

for (curli_measure in unique(curli_features$feature)){
  
  input_df <- curli_features %>% 
    filter(feature == curli_measure)
  plot <- boxplot_all_facet(
    input_df,
    x = input_df$description,
    y = input_df$arcsin_sqrt_relative_abundance,
    title = "", 
    cols = plot.cols,
    ylabel = paste(curli_measure, "\narcsine square root abundance")
  ) + 
  ...
  print(plot)
  ggsave(plot = plot, filename = paste0("data/Amyloidgenic_Protein_Analysis/", 
                                        gsub("[[:punct:]]", "", curli_measure) , ".svg"), 
         width = 6, height = 5)
  
}


write.xlsx(curli_features,
           file = "data/Amyloidgenic_Protein_Analysis/curli-gene-families.xlsx", 
           overwrite = T)

# 
# kos <- phyloseq_objs$KOs.slim %>% abundances() %>% as.data.frame() %>% rownames_to_column()
# 
# ########### KOs ########### 
